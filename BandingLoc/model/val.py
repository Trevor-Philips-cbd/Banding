...
def _padding_2(x):
    ...
    delta = abs(H-W)
    if H < W:
        x = F.pad(x, (0, 0, 0, delta), 'replicate')
    elif H > W:
        x = F.pad(x, (0, delta, 0, 0), 'replicate')
    return x
...
def run(
    ...
    if training:  # called by train.py
        ...
        imgsz = check_img_size(imgsz, s=stride)  # check image size 让imgsz变得可除于stride 关键是看imgsz怎么用的
    ...
    if not training:
        ...
        pad, rect = (0.0, False) if task == "speed" else (0.5, pt)  # square inference for benchmarks 什么时候用square inference(task == speed)
    ...
    for batch_i, (im, im_hsv, targets, paths, shapes) in enumerate(pbar):
        callbacks.run("on_val_batch_start")
        with dt[0]:
            if cuda:
                im = im.to(device, non_blocking=True)
                im_hsv = im_hsv.to(device, non_blocking=True)
                targets = targets.to(device)
            im = im.half() if half else im.float()  # uint8 to fp16/32
            im /= 255  # 0 - 255 to 0.0 - 1.0
            im_hsv = im_hsv.half() if half else im_hsv.float()  # uint8 to fp16/32
            im_hsv /= 255  # 0 - 255 to 0.0 - 1.0
            nb, _, height, width = im.shape  # batch size, channels, height, width



        # _, _, H, W = im.shape #TODO check hsv和im尺寸是否是一样
        # im = _padding_2(im)
        # im, _, _ = _padding(im, 64)

        # im_hsv = _padding_2(im_hsv)
        # im_hsv, _, _ = _padding(im_hsv, 64)
        # Inference
        with dt[1]:
            preds, train_out = model(im, im_hsv) if compute_loss else (model(im, augment=augment), None)

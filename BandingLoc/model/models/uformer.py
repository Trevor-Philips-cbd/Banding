...
class Uformer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        pool0 = self.dowsample_0(conv0) # 320, 64
        conv1 = self.encoderlayer_1(pool0,mask=mask) 
        pool1 = self.dowsample_1(conv1) # 160, 128
        conv2 = self.encoderlayer_2(pool1,mask=mask) 
        pool2 = self.dowsample_2(conv2) # 80, 256
        conv3 = self.encoderlayer_3(pool2,mask=mask)
        pool3 = self.dowsample_3(conv3) # 40, 512
        ...
        bs, hw, c = pool3.shape
        return pool3.view(bs, c, int(hw**0.5), int(hw**0.5))

...
def train(hyp, opt, device, callbacks):
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_dataloader( # 没有使用augment
            val_path,
            imgsz,
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        optimizer.zero_grad()
        for i, (imgs, imgs_hsv, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            callbacks.run("on_train_batch_start")
            ni = i + nb * epoch  # number integrated batches (since train start)
            imgs = imgs.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            imgs_hsv = imgs_hsv.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            with torch.cuda.amp.autocast(amp):
                pred = model(imgs, imgs_hsv)  # forward
            ...
            if f.exists():
                ...
                if f is best:
                    ...
                    results, _, _ = validate.run(
                        ...
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
                        model=attempt_load(f, device).half(), # model 这里用了half. 导致RuntimeError: Input type (float) and bias type (c10::Half) should be the same

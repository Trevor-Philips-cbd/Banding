    ...
    def __init__(
        ...
        if self.rect:
            ...
            for i in range(nb):
                ...
                if maxi < 1: # batch中最大长宽比<1
                    shapes[i] = [maxi, 1]
                elif mini > 1: # batch中最小长宽比>1
                    shapes[i] = [1, 1 / mini]
...
U版YOLOv3和YOLOv5实现主要就是优化了一下如何将比例相近的图片放在一个batch，这样显然填充的就更少一些了，训练起来也就快了一些。
    ...
    def __getitem__(self, index):
        ...
        img_ = img.copy()
        # augment_hsv(img_, hgain=hyp["hsv_h"], sgain=hyp["hsv_s"], vgain=hyp["hsv_v"])
        cv2.cvtColor(img_, cv2.COLOR_BGR2HSV, dst=img_)
        ...
        img_ = np.ascontiguousarray(img_) # C-contiguous 数组：数据按照行优先顺序存储，符合 C 语言的内存布局方式。

        return torch.from_numpy(img), torch.from_numpy(img_), labels_out, self.im_files[index], shapes
    ...
    def collate_fn(batch):
        ...
        im, im_hsv, label, path, shapes = zip(*batch)  # transposed
        for i, lb in enumerate(label):
            lb[:, 0] = i  # add target image index for build_targets()
        return torch.stack(im, 0), torch.stack(im_hsv, 0), torch.cat(label, 0), path, shapes

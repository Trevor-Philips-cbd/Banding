...
def draw_fade_stripe(draw, start_x, stripe_width, width, height, stripe_opacity, mask):
    ...
    for i in range(stripe_width // 2):
        if i < stripe_width * 0.35:
            opacity = int(stripe_opacity * (i / (stripe_width // 2)))  # 从中间向两边透明度增加
        else:
            opacity = int(stripe_opacity * (stripe_width * 0.35) / (stripe_width // 2))
            # print(opacity)
...
def process_image(image_path):
    ...
    mask = Image.eval(v, lambda x: 255 if 0 <= x <= 170 else 0)
    mask = np.array(mask)

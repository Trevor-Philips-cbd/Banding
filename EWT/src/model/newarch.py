...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        q, k, v = qkv[0], qkv[0], qkv[1]  # make torchscript happy (cannot use tensor as tuple)
...
class AFF(nn.Module):
    ...
    def __init__(self, channels=256, r=1):
        super(AFF, self).__init__()
        inter_channels = int(channels // r)

        self.local_att = nn.Sequential(
            nn.Conv2d(channels, inter_channels, kernel_size=3, padding=1),
...
class DoubleBranchBlock(nn.Module):
    ...
    def __init__(self, dim, img_resolution, depth, num_heads, window_size,
        ...
        SIEB_1 = []
        SIEB_1.append(ResBlock(dim, 3))
        SIEB_2 = []
        SIEB_2.append(ResBlock(dim, 3))

        self.SIEB_1 = nn.Sequential(*SIEB_1)
        self.SIEB_2 = nn.Sequential(*SIEB_2)
    ...
    def forward(self, x, x_size):
        H, W = x_size
        B, _, C = x.shape
        x_1_1 = self.SIEB_1(x.transpose(1, 2).reshape(B, C, H, W))
        for fbranch in self.FIEB_1:
            x_1_2 = fbranch(x, x_size)
        x_1 = x_1_2.mul(self.sigmoid(x_1_1.flatten(2).transpose(1, 2))) + x_1_1.flatten(2).transpose(1, 2)
        x_2 = x_1_1.mul(self.sigmoid(x_1_2.transpose(1, 2).reshape(B, C, H, W))) + x_1_2.transpose(1, 2).reshape(B, C, H, W)
        x_2_1 = self.SIEB_1(x_1.transpose(1, 2).reshape(B, C, H, W))
        for fbranch in self.FIEB_2:
            x_2_2 = fbranch(x_2.flatten(2).transpose(1, 2), x_size)
        x = self.fu(x_2_1, x_2_2.transpose(1, 2).reshape(B, C, H, W))
        # return torch.cat((x_2_2, x_2_1.flatten(2).transpose(1, 2)), dim=2)
